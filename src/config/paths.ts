...
export function resolveAgentDir(): string {
  return path.join(resolveConfigDir(), "agent");
}
...
export function resolveSessionFile(sessionName: string): string {
  return path.join(resolveSessionsDir(), `${sessionName}.jsonl`);
}
